...
const COUNTRY_CALLING_CODES = new Set([
  ...
  "82", "84", "86", "90", "91", "92", "93", "94", "95", "98", "211",
  ...
  "800", "808", "850", "852", "853", "855", "856", "870", "878", "880",
  "881", "882", "883", "886", "888", "960", "961", "962", "963", "964",
  "965", "966", "967", "968", "970", "971", "972", "973", "974", "975",
  "976", "977", "979", "992", "993", "994", "995", "996", "998"
]);

const addPhoneCandidate = ({ phoneNumbers, countryCodePairs }, phone, countryCode) => {
  if (!phone) return;

  phoneNumbers.add(phone);

  if (countryCode) {
    countryCodePairs.push({ phone, countryCode });
  }
};

export function processQueryPhoneForLookup(phone) {
  if (!phone) {
    return {
      phoneNumbers: [],
      countryCodePairs: []
  ...
  addPhoneCandidate(result, compactPhone.replace(/^\+/, ""));

  if (compactPhone.startsWith("+")) {
    addPhoneCandidate(result, compactPhone);
  }

  addPhoneCandidate(result, digitsOnly);

  for (let codeLength = 1; codeLength <= 4; codeLength += 1) {
    if (digitsOnly.length <= codeLength) continue;

    const callingCode = digitsOnly.slice(0, codeLength);

    if (!COUNTRY_CALLING_CODES.has(callingCode)) continue;

    const localPhone = digitsOnly.slice(codeLength);
    addPhoneCandidate(result, localPhone, `+${callingCode}`);
  }

  return {
    phoneNumbers: [...result.phoneNumbers].filter(Boolean),
    countryCodePairs: result.countryCodePairs
  };
}
